...
class MockServerRequestHandler(BaseHTTPRequestHandler):
    ...
    def createListFromTrades(self, listTrades, markP):
        ...
        for trade in listTrades:
            #arrayTrades[r, trade.]
            dic = {'markPrice': markP, 'pair': pair, 'price': trade['price'], 'qty':trade['size'],'time': convert_utc_to_epoch_trades(trade['timestamp']), 'isBuyerMaker': int(trade['side'].lower()!='buy')}
            blist.append(dic)
            r = r+1
        return blist
    ...
    def do_GET(self):
        ...
        ltrades = self.createListFromTrades(body, markPrice)
